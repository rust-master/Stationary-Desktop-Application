...
{
    public partial class DailySaleReport : Form
    {
        SqlConnection con;
        ...
        private void DailySaleReport_Load(object sender, EventArgs e)
        {
            AnimateWindow(this.Handle, 500, AnimateWindowFlags.AW_BLEND);
            string date = DateTime.Today.ToString("MM-dd-yyyy");
            rprt.Load(@"C:\Users\Zaryab\source\repos\ResponsiveDesign\ResponsiveDesign\Reports\DailySaleCrystalReport.rpt");
            String selctQRY = "Select * from DailSaleView where  Date = '" + date + "'";
            SqlDataAdapter sda = new SqlDataAdapter(selctQRY, con);
            DataSet ds = new DataSet();
            sda.Fill(ds, "DailSaleView");
            rprt.SetDataSource(ds.Tables["DailSaleView"]);
            dailysalecrystalReportViewer1.ReportSource = rprt;
        }
        ...
        private void DailySaleReport_FormClosing(object sender, FormClosingEventArgs e)
        {
